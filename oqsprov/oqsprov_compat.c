...
int oqsx_ASN1_STRING_length(const ASN1_STRING *x) {
    static int resolved = 0;
    static size_t (*fn_size_t)(const ASN1_STRING *) = NULL;
    static int (*fn_int)(const ASN1_STRING *) = NULL;

    if (!resolved) {
        /* Prefer new OpenSSL 4.1 APIs, fall back to legacy */
        fn_size_t = (size_t (*)(const ASN1_STRING *))oqsx_dlsym(
        ...
        if (!fn_size_t)
            fn_int =
                (int (*)(const ASN1_STRING *))oqsx_dlsym("ASN1_STRING_length");
        resolved = 1;
    }

    if (fn_size_t) {
        size_t len = fn_size_t(x);
        if (len > INT_MAX)
            return -1;
        return (int)len;
    }
